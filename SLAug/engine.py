...
def align_features(model, proj, tar_patches, slaug_textfeat):
    B, P, E = proj.shape
    PLoss_per_image = []
    NLoss_per_image = []
...
def train_one_epoch_SBF(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    slaug_textfeat = torch.load("/l/users/shahina.kunhimon/Language_grounding/TEXT_FEATURES/organ_vectors_only.pt")
    for i, samples in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        for k, v in samples.items():
            if isinstance(samples[k], torch.Tensor):
                samples[k] = v.to(device)
        
        GLA_img = samples['images'] #(32,1,192,192)
        ...
        input_var = Variable(GLA_img, requires_grad=True) #(32,1,192,192)
        optimizer.zero_grad()


        logits, proj = model(input_var) #([32, 5, 192, 192])#([32, 36, 512])

        ############ patchify segmn label mask############
        tar_patches = rearrange(lbl,'b (h q) (w r) -> b (h w) (q r)',q=32, r=32, h=6, w=6) #(32,36,512)

        B,P,E = proj.shape #B=32, P=36,E=512
        ###############ours##############################################
        # PLoss_per_image=[]
        # NLoss_per_image=[]
        # for i in range(B):
        ...
        #             for p in pos_labels:
        #                 plabel_text_feat = slaug_textfeat[p-1].cuda()
        #                 cos_dist = torch.nn.functional.cosine_similarity(proj[i,j],plabel_text_feat, dim=0) #(1*1)
        #                 ploss_per_label=torch.max(torch.tensor(0.0),1-cos_dist)
        #                 pos_labels_loss.append(ploss_per_label)
        #             lpos_per_patch = torch.mean(torch.stack(pos_labels_loss,dim=0))
        #             L_pos.append(lpos_per_patch)
        ...
        #                 nlabel_text_feat=slaug_textfeat[n-1].cuda()
        #                 cos_dist=torch.nn.functional.cosine_similarity(proj[i,j],nlabel_text_feat, dim=0)
        #                         #nloss_per_label=torch.max(torch.tensor(0.0).cuda(),1 + cos_dist)
        #                 nloss_per_label=torch.max(torch.tensor(0.0),1 + cos_dist)
        #                 neg_labels_loss.append(nloss_per_label)
        ...
        # Loss_neg=torch.mean(torch.stack(NLoss_per_image,dim=0))
        # Loss_local = Loss_pos + Loss_neg  
        # Loss_local=(Loss_local/B)
        # print("LOSS_LOCAL=",Loss_local)   
        ###############ours##############################################
        Loss_local = align_features(model, proj, tar_patches, slaug_textfeat)
        Loss_local=Loss_local/B
        #SLAUG LOSS
        ...
        aug_logits,aug_proj = model(aug_var)
        aug_Loss_local = align_features(model, aug_proj, tar_patches, slaug_textfeat)
        aug_Loss_local=aug_Loss_local/B
        ...
        aug_losses = aug_losses + aug_Loss_local
        aug_losses.backward()
