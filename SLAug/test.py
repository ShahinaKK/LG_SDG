...
class CustomModel(nn.Module):
      def __init__(self, base_model):
        ...
        self.linear = nn.Linear(352,512)

      def forward(self,x):
        encoder_outputs = self.base_model.encoder(x)                                                              #(32,352,6,6)^[OA
        ...
        flat_enc= enc_reprn.view(enc_reprn.size(0), -1, enc_reprn.size(1))   # Flatten the output to (batch_size, 36 * 352)^[OA
...
if __name__ == "__main__":
    ...
    eval_mode = True
    show_config = False
    base_model = instantiate_from_config(model_config)
